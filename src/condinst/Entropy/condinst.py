...
import random
EPS = 1e-12
def sampled_points_with_uncertainty(pred_score, ins_box):
    x = pred_score.mean(0)
    uncertainty = - x * torch.log(x + EPS) - (1 - x) * torch.log(1 - x + EPS)

    x0,y0,x1,y1 = ins_box.tolist()
    keep = torch.zeros_like(uncertainty).to(uncertainty.device)
    keep[y0:y1, x0:x1] = 1
    uncertainty *= keep

    points = (uncertainty==torch.max(uncertainty)).nonzero()
    random_idx = random.randint(0, len(points)-1)
    point = points[random_idx]
    return point
...
class CondInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # pointsup configs
        self.point_sup_enabled = cfg.INPUT.POINT_SUP
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            mask_losses, preds = self._forward_mask_heads_train(proposals, mask_feats, gt_instances)

            for per_im, per_im_gt_instances, pred in zip(batched_inputs, gt_instances, preds):
                if len(per_im_gt_instances) == 0 or len(pred) == 0:
                    continue

                img_id = per_im['file_name'].split('/')[-1].split('.')[0]
                img_h, img_w = per_im['height'], per_im['width']
                resized_h, resized_w = per_im_gt_instances.image_size
                if min(resized_h, resized_w) == 800:
                    factor = min(resized_h, resized_w) * 1.0 / min(img_h, img_w)
                else:
                    factor = max(resized_h, resized_w) * 1.0 / max(img_h, img_w)
                
                gt_inds, pred_global_masks = pred
                pred_global_masks = F.interpolate(
                    pred_global_masks,
                    size=(img_h, img_w),
                    mode="bilinear", align_corners=False
                )
                pred_global_masks = pred_global_masks[:, 0, :, :] # original image size

                all_points = []
                for ins_idx in gt_inds.unique():
                    pred_score = pred_global_masks[gt_inds==ins_idx]
                    ins_box = torch.floor(per_im_gt_instances.gt_boxes[ins_idx.item()].tensor[0] / factor).int()
                    sampled_points = sampled_points_with_uncertainty(pred_score, ins_box)
                    all_points.append((ins_idx.cpu().data, sampled_points.cpu().data))
                torch.save(all_points, os.path.join(os.getenv('ROOT_PATH'), f'AdelaiDet/points/{img_id}.pt'))
    ...
    def postprocess(self, results, output_height, output_width, padded_im_h, padded_im_w, mask_threshold=0.5):
        ...
        if results.has("pred_global_masks"):
            ...
            if self.point_sup_enabled:
                # filter out any mask prediction outside of predicted boxes (see PointSup)
                pred_boxes = results.pred_boxes.tensor
                for i in range(pred_global_masks.size(0)):
                    kept_mask = torch.zeros_like(pred_global_masks[0]).to(pred_boxes.device)
                    x0,y0,x1,y1 = int(pred_boxes[i][0]),int(pred_boxes[i][1]),int(pred_boxes[i][2]),int(pred_boxes[i][3])
                    kept_mask[y0:y1, x0:x1] = 1
                    pred_global_masks[i] *= kept_mask
