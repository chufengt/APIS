...
SMOOTH = 1e-6
def get_mask_iou(outputs, labels):
    outputs = (outputs>0.5).clone().detach().long()
...
# from detectron2.projects.point_sup.point_utils import get_point_coords_from_point_annotation
def get_point_coords_from_point_annotation(instances, gt_inds, image_wh):
    # re-designed for condinst, image_wh (width, height) should be the padded size
    gt_point_coords = []
    gt_point_labels = []

    for per_im in instances:
        gt_point_coords.append(per_im.gt_point_coords.to(torch.float32) / image_wh)
        gt_point_labels.append(per_im.gt_point_labels.to(torch.float32).clone())

    gt_point_coords = torch.cat(gt_point_coords)
    gt_point_labels = torch.cat(gt_point_labels)

    gt_point_coords = gt_point_coords[gt_inds]
    gt_point_labels = gt_point_labels[gt_inds]

    return gt_point_coords, gt_point_labels
...
class DynamicMaskHead(nn.Module):
    def __init__(self, cfg):
        ...
        self.point_loss_weight = cfg.MODEL.BOXINST.POINT_LOSS_WEIGHT
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None):
        if self.training:
            ...
            if not self.boxinst_enabled and not self.point_sup_enabled:
                # fully-supervised
                losses["loss_mask"] = dummy_loss
            else:
                # BoxInst and/or PointSup settings
                if self.boxinst_enabled:
                    losses["loss_prj"] = dummy_loss
                    losses["loss_pairwise"] = dummy_loss
                if self.point_sup_enabled:
                    losses["loss_point"] = dummy_loss
            ...
            if len(pred_instances) != 0:
                ...
                resized_im_h, resized_im_w = gt_instances[0].image_size
                pred_global_masks = aligned_bilinear(
                    mask_scores, int(self.mask_out_stride)
                )
                pred_global_masks = pred_global_masks[:, :, :resized_im_h, :resized_im_w]
            
                preds = (gt_inds, pred_global_masks)
            ...
            #     else:
            #         # BoxInst and/or PointSup losses
            #         if self.boxinst_enabled:
            ...
            #             })
            #         if self.point_sup_enabled:
            #             # pointly-supervised CondInst losses
            #             image_wh = torch.Tensor([mask_logits.size(3) * self.mask_out_stride, mask_logits.size(2) * self.mask_out_stride]).to(mask_logits.device)
            #             point_coords, point_labels = get_point_coords_from_point_annotation(gt_instances, gt_inds, image_wh)
            ...
            #             loss_point = roi_mask_point_loss(point_logits, gt_instances, point_labels)

            #             losses.update({
            #                 "loss_point": loss_point * self.point_loss_weight,
            #             })
            # return losses
        else:
            if len(pred_instances) > 0:
